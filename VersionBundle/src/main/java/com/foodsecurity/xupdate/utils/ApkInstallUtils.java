...
import com.foodsecurity.xupdate.utils.ShellUtils.CommandResult;
...
public final class ApkInstallUtils {
    ...
    private static boolean installNormal(Context context, File appFile) {
        try {
            ...
            UpdateFacade.onUpdateError(INSTALL_FAILED, "使用系统的意图进行apk安装失败！");
        }
        return false;
    }
    ...
    public static Intent getInstallAppIntent(Context context, File appFile) {
        try {
            ...
            UpdateFacade.onUpdateError(INSTALL_FAILED, "获取安装的意图失败！");
        }
        return null;
    }
