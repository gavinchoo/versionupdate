...
package com.foodsecurity.xupdate.service;
...
public class DownloadService extends Service {
    ...
    private void startDownload(@NonNull UpdateEntity updateEntity, @NonNull FileDownloadCallBack fileDownloadCallBack) {
        ...
        String apkName = updateEntity.getFileName();
        ...
        String target = apkCacheDir + File.separator;
        long fileSize = updateEntity.getDownLoadEntity().getSize();
        UpdateLog.d("开始下载更新文件, 下载地址:" + apkUrl + ", 保存路径:" + target + ", 文件名:" + apkName + ", 文件大小:" + fileSize);
        updateEntity.getIUpdateHttpService().download(apkUrl, target, apkName, fileSize, fileDownloadCallBack);
    }
    ...
    private class FileDownloadCallBack implements IUpdateHttpService.DownloadCallback {
        ...
        public void onProgress(long total, float progress) {
            ...
            if (mOldRate != rate) {
                if (mOnFileDownloadListener != null) {
                    mOnFileDownloadListener.onProgress(total, progress);
                }
        ...
        public void onSuccess(File file) {
            ...
            try {
                if (UpdateUtils.isAppOnForeground(DownloadService.this)) {
                    ...
                    if (mIsAutoInstall) {
                        UpdateFacade.startInstallApk(DownloadService.this, file, mDownloadEntity);
                    } else {
